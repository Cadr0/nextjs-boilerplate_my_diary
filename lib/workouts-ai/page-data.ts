...
export function readSelectedWorkoutDate(value: string | string[] | null | undefined) {
...
export function createEmptyWorkoutsSidebarData(selectedDate: string): WorkoutsSidebarData {
  return {
    selectedDate,
    activeSession: null,
    days: [
...
export async function loadWorkoutsPageData(userId: string, selectedDate: string) {
  const trace = createServerPerfTrace("workouts.page.data");
  const { sidebarData, sessionDetails } = await trace.measure("sidebar", () =>
    loadSidebarData(userId, selectedDate),
  );
